...
exports.findByName = function(req, res){
  ...
  if(name)
  {
     Musician.find({'name':new RegExp('^.*'+name+'.*$', "i")},function(err, result) {        
 // Musician.find({'name':name},function(err, result) {
    return res.send(result);
  });
  } 
};
exports.findByBand = function(req, res){
  var band = req.params.band;
  if(band)
  {
     Musician.find({'band':new RegExp('^.*'+band+'.*$', "i")},function(err, result) {        
 // Musician.find({'name':name},function(err, result) {
    return res.send(result);
  });
  }  
